...
app.get("/last", (req, res)=>{
  ...
  app.post("/view", async (req, res)=>{
    let outputFile = "D:/btech/6th Sem/minorproject/pdf_maker/src/";
...
app.post("/form", async (req, res) => {
    ...
    let syllabus = "D:/btech/6th Sem/minorproject/pdf_maker/src/syllabus/" + subVal+".pdf";
  
    const urls = ["/1_doc", "/2_doc", "/10_doc"];
    const pdfs = ["D:/btech/6th Sem/minorproject/pdf_maker/src/pg34.pdf"];
    ...
    if(time_table!="") await merger.add(time_table);
    if(lect_plan!="" ) await merger.add(lect_plan);
    if(attend!="") await merger.add(attend);
    if(assig!="") await merger.add(assig);
    if(q_paper!="") await merger.add(q_paper);
    // if(q_paper!=null || q_paper!=undefined) await merger.add(q_paper);
    if(marks!="") await merger.add(marks);
    if(summary_sheet!="") await merger.add(summary_sheet);
